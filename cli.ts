...
import { generateZodClientFromOpenAPI } from "./lib/src/generateZodClientFromOpenAPI.ts";
import { toBoolean } from "./lib/src/utils.ts";
...
try {
    // Check if running from a file URL (local) or https URL (JSR)
    if (import.meta.url.startsWith("file://")) {
        const scriptDir = dirname(fromFileUrl(import.meta.url));
        const denoConfigPath = join(scriptDir, "deno.json");
        const jsonString = await Deno.readTextFile(denoConfigPath);
        const config = JSON.parse(jsonString);
        version = config.version || version;
    } else if (import.meta.url.startsWith("https://")) {
        // Running from JSR - fetch deno.json from the same URL
        const baseUrl = new URL(".", import.meta.url);
        const denoConfigUrl = new URL("deno.json", baseUrl);
        const response = await fetch(denoConfigUrl.toString());
        if (response.ok) {
    ...
    // Fallback to "-" if deno.json can't be read
